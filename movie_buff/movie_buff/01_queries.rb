...
def harrison_ford
  ...
  Movie.joins(:actors, :castings).where("actors.name = 'Harrison Ford' AND castings.ord != 1").distinct.select(:id,:title)
end
...
def directed_by_one_of(them)
  ...
  Movie.joins('JOIN actors ON movies.director_id = actors.id').where(actors: {name: them}).select(:id, :title)
end
